<template>
  ...
  <my-calculator @addToHistory="updateHistory" :historyCalc="previousCalc" />
  ...
  <div v-for="(calc, index) in pastCalculations" :key="calc.previous" class="flex justify-center space-y-5">
      <div class="border-2 text-2xl mt-2 rounded-lg border-black w-32 text-center hover:border-green-400" @click="previousCalc = index">
        <p>
          {{ calc.previous }}
          {{ calc.operator }}
          {{ calc.query }}
